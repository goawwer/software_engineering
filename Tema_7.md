...
В данном задании я придумал собственное условие, благодаря которому, как мне кажется, повторил свои знания, связанные с листом и добавлением внего кортежа. Также познакомился с новым способом поиска максимального и минимального значений по ключу с помощью max(lines, key=len). А также узнал о существовании isdigit, которая отлавливает наличие числа в строке.

## Общие выводы по теме

Данная тема наглядно показала мне, как правильно нужно работать с файлами в python. 

Я научился нескольким новым позеным функциям, например, with open('file', 'r/a/w') as f, которая позволяет удобно работать с файлыми с функцией автоматического закрытия. Также узнал про понятие encoding, как создавать csv файлы, как работать с модулем os, различными функциями, которые упрощяют поиск элементов, как max(lines, key=len). 

Данная тема не только закрепила мои прошлые знания, но и добавило довольно много новых инструментов в мою работу.

...
В данном задании я создал класс с пустым конструктором и объект "my_home".
...
Данное задание дало возможность намного лучше разобраться в работе с классами, а именно с их атрибутами. Я прописал всё, что мне требовалось в конструкторе и с помощью функции "show_info" вывел информацию в output. 
...
В этом задании я разобрался с наследованием в Python. Функция super(). позволяет наследовать атрибуты у "родителя". Также я придумал собственную интересную программу, которая построена на логике ООП.
...
Данная программа осложняется интересной конструкцией геттера и сеттера. В питон существует возможность создать приватный атрибут, как я понял, для более безопасной работы самой программы. С помощью геттера и сеттера можно добираться до этих скрытых (приватных) для пользователя атрибутов. 
...
В данной программе я изучил понятие полиморфизма и воспользовался в своем коде главной его концепцией - объекты разных классов используют один и тот же интерфейс для различных действий. Здесь я придумал программу с различными классами подписок и они все проходят через метод pricing, который уникален для каждого класса. 

## Общие выводы по теме

Данная тема позволяет наглядно ознакомиться с ООП в Python. 

Я наконец-то начал понимать суть данного паттерна программирования, и нахожу его действительно полезным и удобным. Разбиение на классы до этого давалось мне немного трудно, особенно понятие "наследование" - казалось мне страшным сном.

Но всё же я прошёл этот путь и теперь я знаком с консруктором класса "def __init__(self, ...):", созданием атрибутов, функцией наследования атрибутов от родителя "super().__init__(...)", созданием приватных атрибутов через запись с "__", сеттерами и гетерами и полиморфизмом. 
